...
var ClassicServiceTemplate = `// Code generated by furo-proto-gen. DO NOT EDIT.
...
// Autogenerated request type messages for the mehthods
{{- range $Service := .Services}}
{{- $serviceName := camelcase (snakecase $Service.Name)}}
{{- $rpcmap := $Service.Services | rpcmap}}
{{range $rpckey, $method := $rpcmap}}
// request message for {{if $method.RpcName}}{{$method.RpcName}}{{else}}{{$rpckey}}{{$serviceName}}{{end}}
message {{$rpckey}}{{$serviceName}}Request{ 
 {{- $params := $method | requestParamsFromRpc}}
 {{- range $i, $p := $params}}
 // {{$p.Description | replace "\n" "\n// " | noescape}}
  {{$p.Type}} {{$p.Name}}  = {{$p.FieldId}};
 {{- end}}
}
{{end}}
{{end}}
